...
EMAIL_SUBJECT = "[SIT-LAKEHOUSE] Orphaned and Unknown Table and Folder Cleanup"
...
def send_email_notification(summary):
    ...
    html_body = "<html><body><h3>Cleanup Summary for All SIT Lakehouses</h3>"
    for cat_name, deleted in summary.items():
        ...
        html_body += "<tr><th>Namespace</th><th>Detected Orphaned Folder and Cleaned</th></tr>"
        if deleted:
            for ns, folders in deleted.items():
                if ns == "__unknown_root_folders__":
                    ...
                    ns_label = ns
                html_body += f"<tr><td>{ns_label}</td><td>{'<br>'.join(folders)}</td></tr>"
        else:
            html_body += "<tr><td colspan='2'>No orphaned or unknown folder found.</td></tr>"
        html_body += "</table><br>"
    html_body += "</body></html>"
