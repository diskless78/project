...
class SparkConfig:
    ...
    def get_config_map(catalog_name: str, nessie_branch: str) -> dict:
        if catalog_name == "lakehouse_platform":
            lakehouse_bucket = SparkConfig.get_env('LAKEHOUSE_PLATFORM_BUCKET')
            nessie_endpoint = SparkConfig.get_env('NESSIE_ENDPOINT_PLATFORM')
            nessie_token = SparkConfig.get_env('NESSIE_TOKEN_PLATFORM')
            s3_accesskey = SparkConfig.get_env('LAKEHOUSE_PLATFORM_ACCESSKEY')
            s3_secretkey = SparkConfig.get_env('LAKEHOUSE_PLATFORM_SECRETKEY')
