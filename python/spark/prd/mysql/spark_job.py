...
from lib.database.mysql_reader import MYSQLReader
...
def read_data(spark, mysql_cred: dict, sql_query: str, remove_columns: list, rename_columns: list, add_columns: list, convert_columns: list) -> DataFrame:
    try:
        mssql_reader = MYSQLReader(spark=spark, conf=mysql_cred)
        ...
        logger.info("Successfully read data from MYSQL. Row count: %s", df.count())
        return df
    except Exception as e:
        logger.error("Failed to read data from MYSQL: %s", e)
        raise
...
def run_job(source_table_name: str):
    ...
    try:
        ...
        creds = MYSQLReader.load_credentials(CRED_PATH)
        mysql_cred = {
            "host": creds["MYSQL_HOST"],
            "port": int(creds.get("MYSQL_PORT")),
        ...
        if load_start_year is None:  # Case: Table < 5M records
            ...
            if not existing_table:
                schema_query = schema_query = f"SELECT * FROM {source_table_name} LIMIT 1"
                df_schema = read_data(spark, mysql_cred, schema_query, remove_columns, rename_columns, add_columns, convert_columns)
            ...
            df = read_data(spark, mysql_cred, sql_query, remove_columns, rename_columns, add_columns, convert_columns)
