...
from lib.database.oracle_reader import OracleReader
...
def read_data(spark, oracle_cred: dict, sql_query: str, remove_columns: list, rename_columns: list, add_columns: list, sort_by: list) -> DataFrame:
    try:
        oracle_reader = OracleReader(spark=spark, conf=oracle_cred)
        df = oracle_reader.query_table(sql_query)
        ...
        logger.info("Successfully read data from Oracle. Row count: %s", df.count())
        return df
    except Exception as e:
        logger.error("Failed to read data from Oracle: %s", e)
        raise
...
def run_job(source_table_name: str):
    ...
    try:
        ...
        # Configure oracle_reader
        creds = OracleReader.load_credentials(CRED_PATH)
        
        oracle_cred = {
            "host": creds["ORACLE_HOST"],
            "port": int(creds["ORACLE_PORT"]),
            "username": creds["ORACLE_USERNAME"],
            "password": creds["ORACLE_PASSWORD"],
            "service_name": creds["ORACLE_DB"]
        }
        ...
        if load_start_year is None:  # Case: Table < 5M records
            ...
            if not existing_table:
                schema_query = f"SELECT * FROM {source_table_name} WHERE ROWNUM = 1"
                df_schema = read_data(spark, oracle_cred, schema_query, remove_columns, rename_columns, add_columns, sort_by)
            ...
            df = read_data(spark, oracle_cred, sql_query, remove_columns, rename_columns, add_columns, sort_by)
            ...
            if not existing_table:
                ...
                min_value = oracle_reader.execute_query(min_value_query)
                max_value = oracle_reader.execute_query(max_value_query)
                ...
                if load_start_month is None: # Pull year by year
                    ...
                    for year in range(load_start_year, current_year + 1):
                        ...
                        for month in range(start_month_range, end_month_range):
                            sql_query = f"SELECT * FROM {source_table_name} WHERE EXTRACT (YEAR FROM {query_key}) = {year} AND EXTRACT (MONTH FROM {query_key}) = {month}"
                ...
                if incremental_month - 1 == 0:
                    if 1 <= date.today().day <= 7:
                        if start_year != end_year:
                            sql_query = f"SELECT * FROM {source_table_name} WHERE (EXTRACT (YEAR FROM {query_key}) = {start_year} AND EXTRACT (MONTH FROM {query_key}) = {start_month}) OR (EXTRACT (YEAR FROM {query_key}) = {end_year} AND EXTRACT (MONTH FROM {query_key}) = {end_month})"
                            logger.info(f"Table already exists. Performing incremental from month: {start_month}/{start_year} and {end_month}/{end_year}")
                        else:
                            sql_query = f"SELECT * FROM {source_table_name} WHERE EXTRACT (YEAR FROM {query_key}) = {start_year} AND EXTRACT (MONTH FROM {query_key}) BETWEEN {start_month} AND {end_month}"
                            logger.info(f"Table already exists. Performing incremental from month: {start_month} to {end_month} in year: {start_year}")
                    else:
                        sql_query = f"SELECT * FROM {source_table_name} WHERE EXTRACT (YEAR FROM {query_key}) = {end_year} AND EXTRACT (MONTH FROM {query_key}) = {end_month}"
                        logger.info(f"Table already exists. Performing incremental from month: {end_month} in year: {start_year}")
                else: 
                    if start_year != end_year:
                        sql_query = f"SELECT * FROM {source_table_name} WHERE (EXTRACT (YEAR FROM {query_key}) = {start_year} AND EXTRACT (MONTH FROM {query_key}) = {start_month}) OR (EXTRACT (YEAR FROM {query_key}) = {end_year} AND EXTRACT (MONTH FROM {query_key}) = {end_month})"
                        logger.info(f"Table already exists. Performing incremental from month: {start_month}/{start_year} and {end_month}/{end_year}")
