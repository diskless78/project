...
def write_sqlquery(table_name, query_key, start_dt, end_dt):
    ...
    sql_query = (
        ...
        f"WHERE {query_key} >= TO_DATE('{start_date_str}', 'YYYY-MM-DD HH24:MI:SS') "
        f"AND {query_key} < TO_DATE('{end_date_str}', 'YYYY-MM-DD HH24:MI:SS')"
    )
...
def read_data(spark, oracle_cred: dict, sql_query: str, remove_columns: list, rename_columns: list, add_columns: list, convert_columns: list) -> DataFrame:
    try:
    ...
    try:
        ...
        if load_start_year is None:  # Case: Table < 5M records
            ...
            if not existing_table:
                ...
                df_schema = read_data(spark, oracle_cred, schema_query, remove_columns, rename_columns, add_columns, convert_columns)
            ...
            df = read_data(spark, oracle_cred, sql_query, remove_columns, rename_columns, add_columns, convert_columns)
