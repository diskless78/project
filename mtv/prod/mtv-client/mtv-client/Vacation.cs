...
namespace mtv_client
{
    public partial class Vacation : Form
    {
        ...
        private void AddVacationToListBox()
        {
            ...
            if (!FindTheLuckThenFuckTheLife.IsThePublicHoliday(selectedDate))
            {
        ...
        {
            string filePath = AppSetting.UserVacationFolder + @"\vacation_" + enteredEmailAddress.Split('@')[0] + ".txt";
            ...
            if (File.Exists(filePath))
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    if (parts.Length == 2)
                    {
                        ...
                        {
                            FindTheLuckThenFuckTheLife.LogMessage("Invalid date format: " + dateStr, AppSetting.errorLogFile);
                        }
                    }
                }
            }
        ...
        {
            bool matchPassword = FindTheLuckThenFuckTheLife.IsPasswordMatched(txt_Password.Text, txt_EmailAddress.Text.Trim().ToLower());
        ...
        private void saveDateOff_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string userVacationDataFolder = Path.Combine(AppSetting.DataSharedFolder, $"vacation_{emailPrefix}.txt");
                string userVacationDestFolder = Path.Combine(AppSetting.UserVacationFolder, $"vacation_{emailPrefix}.txt");
